...
import {
  ...
  FETCH_WALLET_ETHEREUM_BALANCES,
  SET_WALLET_ADDRESS,
  RESET,
  SET_NETWORK,
} from '../types/Wallet';
...
export function unlockExtraWallet(strategy, data) {
  return async function (dispatch, getState) {
    let wallet = null;

    try {
      wallet = await Wallet.unlockExtraWallet(strategy, data);
    } catch (e) {
...
export function logoutWalletOnly() {
  return async function (dispatch, getState) {
    //Clear the wallet
    Wallet.logoutWallet();
