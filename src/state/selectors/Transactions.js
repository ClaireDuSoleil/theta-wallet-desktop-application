...
const getEthereumNetworkTransactionsByType = (state) =>
  state.transactions.ethereumNetworkTransactionsByType;

const getWalletAddress = (state) => Wallet.getWalletAddress();

function sortThetaNetworkTransactionOutputs(walletAddress, outputs) {
  return _.sortBy(outputs, [
    function (output) {
...
function transformThetaNetworkTransaction(walletAddress, transaction) {
  try {
    ...
    console.log('exception in transformThetaNetworkTransaction -- ignore and try again ');
    return null;
  }
}
...
function getTransformedEthereumNetworkTransactions(
  walletAddress,
  type,
  transactionsByType,
...
export const getERC20Transactions = createSelector(
  ...
  (walletAddress, transactionsByType, localTransactionsByHash) => {
    return getTransformedEthereumNetworkTransactions(
      walletAddress,
      TokenTypes.ERC20_THETA,
      transactionsByType,
      localTransactionsByHash
    );
  }
