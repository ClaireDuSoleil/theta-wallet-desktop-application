...
const ThetaBaseMnemonicPath = "m/44'/500'/0'/0/";
...
export default class Wallet {
  ...
  static setWallet(wallet) {
    ...
    if (wallet === null) {
      ...
      Wallet._mnemonic = null;
      Wallet._sessionPassword = null;
    }
  }
  ...
  static unlockWalletFromExtraAddress(serializedPath) {
    let extraWallet = ethers.Wallet.fromMnemonic(this._mnemonic, serializedPath);
    return extraWallet;
  }
  ...
  static walletExtraAddressesAvailable() {
    if (this._mnemonic != null) return true;
    return false;
  }

  static getPageExtraAddresses(page) {
    let addresses = [];
    try {
      for (let i = 0; i < NumPathsPerPage; i++) {
        let extraWallet = ethers.Wallet.fromMnemonic(
          this._mnemonic,
          ThetaBaseMnemonicPath + (page * NumPathsPerPage + i).toString()
        );
        if (extraWallet) {
          let address = {address: extraWallet.address, serializedPath: extraWallet.path};
          addresses.push(address);
        }
      }
    } catch (err) {
      console.log('caught an error in Wallet::getPageExtraAddresses :', err.message());
      console.log(ThetaBaseMnemonicPath + (page * NumPathsPerPage + 0).toString());
    }
    return addresses;
  }

  static walletFromMnemonic(mnemonic, password) {
    //the following two lines are *only* for the wallet desktop app, not the web app
  ...
  static createWallet(password) {
    ...
    let wallet = this.walletFromMnemonic(mnemonic, password);
  ...
  static async unlockExtraWallet(strategy, data) {
    return this.unlockWallet(strategy, this._sessionPassword, data);
  }
  ...
  static async unlockWallet(strategy, password, data) {
    ...
    try {
      if (strategy === WalletUnlockStrategy.KEYSTORE_FILE) {
        ...
        wallet = Wallet.walletFromMnemonic(mnemonic.toString(), password);
      ...
      if (wallet) {
        ...
        if (
          strategy !== WalletUnlockStrategy.COLD_WALLET &&
          (keystore === null || keystore === undefined)
        ) {
